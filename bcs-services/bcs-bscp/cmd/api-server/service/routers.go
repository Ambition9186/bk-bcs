...
func (p *proxy) routers() http.Handler {
	...
	// 导入模板压缩包
	r.Route("/api/v1/config/biz/{biz_id}/template_spaces/{template_space_id}/templates/import/{fiename}",
		func(r chi.Router) {
			r.Use(p.authorizer.UnifiedAuthentication)
			r.Use(p.authorizer.BizVerified)
	...
	// 导入配置压缩包
	r.Route("/api/v1/config/biz/{biz_id}/apps/{app_id}/config_item/import/{fiename}", func(r chi.Router) {
		r.Use(p.authorizer.UnifiedAuthentication)
		r.Use(p.authorizer.BizVerified)
